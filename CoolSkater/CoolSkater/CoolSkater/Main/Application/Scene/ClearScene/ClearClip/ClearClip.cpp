...
bool ClearClip::Initialize()
{
	...
	if (!SINGLETON_INSTANCE(Lib::TextureManager)->LoadTexture(
		"Resource\\ClearScene\\Cliping.png",
		&m_TextureIndex))
	{
		return false;
	}
	...
	if (!SINGLETON_INSTANCE(Lib::TextureManager)->LoadTexture(
		"Resource\\ClearScene\\Cliping2.png",
		&m_TextureIndex2))
	{
		return false;
	}
	...
	if (!SINGLETON_INSTANCE(Lib::ShaderManager)->LoadVertexShader(
		"Resource\\ClearScene\\ClipEffect.fx",
		"VS",
		&m_VertexShaderIndex))
	{
		return false;
	...
	if (!SINGLETON_INSTANCE(Lib::ShaderManager)->LoadVertexShader(
		"Resource\\ClearScene\\TextureCreateEffect.fx",
		"VS",
	...
	for (int i = 0; i < 10; i++)
	{
		m_ClipSize[i] = D3DXVECTOR2(0, 0);
		m_ClipPos[i] = D3DXVECTOR2(
			static_cast<float>(m_MersenneTwister() % 1280),
			static_cast<float>(m_MersenneTwister() % 1000));

		m_ClipTime[i] = m_MersenneTwister() % 30;
	}

	m_ClipPos[0] = D3DXVECTOR2(100, 320);
	m_ClipTime[0] = 10;
	m_ClipPos[1] = D3DXVECTOR2(1000, 830);
	m_ClipTime[1] = 30;
	m_ClipPos[2] = D3DXVECTOR2(400, 1300);
	m_ClipTime[2] = 15;
	m_ClipPos[3] = D3DXVECTOR2(1200, 100);
	m_ClipTime[3] = 20;
...
void ClearClip::Update()
{
	...
	else
	{
		for (int i = 0; i < 10; i++)
		{
			if (m_ClipTime[i] <= 0)
			{
				m_ClipSize[i] = D3DXVECTOR2(m_ClipSize[i].x + 10, m_ClipSize[i].y + 10);
			}
			else
			{
				m_ClipTime[i]--;
...
void ClearClip::Draw()
{
	...
	for (int i = 0; i < 10; i++)
	{
		m_pVertex->SetVertex(&m_ClipSize[i]);
		m_pVertex->WriteVertexBuffer();
		...
		pContext->DSSetShader(nullptr, nullptr, 0);
		pContext->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLESTRIP);
		m_pVertex->WriteConstantBuffer(&m_ClipPos[i]);
		m_pVertex->Draw();
	}
